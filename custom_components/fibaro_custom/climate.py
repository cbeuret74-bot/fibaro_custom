...
class FibaroThermostat(FibaroDevice, ClimateEntity):
                    ...
                    try:
                        if device.type != "com.fibaro.hvacSystemHeat":
                            ...
                            if mode not in HEATMODES:
                                _LOGGER.warning("%d unknown heat mode", HEATMODES[mode])
                                continue
    ...
    def fibaro_op_mode(self) -> str | int:
        ...
        if device.has_thermostat_mode:
            if device.type != "com.fibaro.hvacSystemHeat":
                ...
                return HEATMODES[device.thermostat_mode]
        return device.mode
    ...
    def hvac_mode(self) -> HVACMode | None:
        ...
        _LOGGER.debug("%s return hvac mode %s", self.ha_id, fibaro_operation_mode)
                ...
                for mode in device.supported_thermostat_modes:
                    if device.type != "com.fibaro.hvacSystemHeat":
                        ...
                        if mode == HA_HEATMODES[hvac_mode]:
                            # _LOGGER.debug("callAction hvac mode %s", mode)
                            self._op_mode_device.callAction("setThermostatMode", mode)
                            break
